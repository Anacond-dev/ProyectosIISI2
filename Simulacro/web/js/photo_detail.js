...
async function loadPhotoDetails() {
    try {
        ...
        // Aqui obtenemos todas las fotos de la base de datos y con un find cogemos la primera que encontramos segun el id de la foto que tenemos en la pagina

        //En este caso no tenemos que filtrar por usuario porque todos los usuario tienen el mismo voto medio

        let vs = await v_votesAPI_auto.getAll();

        //IMPORTANTE: cuando vayamos a utilizar finds usar siempre Number o otras declaraciones de tipos, porque podemos estar comparando un tipo number con un string y eso da error

        let average = vs.find(v => Number(v.photoId) === Number(photoId)).avgValue;

        //Cogemos la etiqueta donde ira el voto publico

        let rating = document.getElementById("public-vote");

        //Cambiamos el contenido de la etiqueta html
